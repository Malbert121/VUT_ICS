...
namespace DAL_Tests
{
    public class Activity_and_Rating_Test
    {
        ...
        public async Task Activity_Add_new()
        {
            ...
            using (var context = new SchoolContext(new DbContextOptions<SchoolContext>()))
            {
                var new_activity = await context.Activities.SingleAsync(i => i.Id == entity.activityId);
        ...
        public async Task Activity_Delete()
        {
            ...
            using (var context = new SchoolContext(new DbContextOptions<SchoolContext>()))
            {
                ...
                var new_activity = await context.Activities.SingleAsync(i => i.activityId == entity.Id);
                Assert.NotNull(new_activity);
                Assert.Equal(entity.activityId, new_activity.activityId);
