...
public static class DeepAssert
{
    ...
    public static void Contains<T>(T expected, IEnumerable<T>? collection, params string[] propertiesToIgnore)
    {
        ...
        if (!collection.Any(item => compareLogic.Compare(expected!, item).AreEqual) && expected != null)
        {
            string? expect = expected.ToString();
            if (expect != null)
            {
                throw ContainsException.ForCollectionItemNotFound(expect, nameof(collection));
            }
        }
    }
}
